...
## 3. 빌더 패턴
점층적 생성자 패턴의 안전성과 자바비즈 패턴의 가독성을 가지고 있다.<br>
필수 매개변수만으로 생성자를 호출해 빌더 객체를 얻는다.<br>
그 다음 내부 메서드들을 통해 원하는 선택 매개변수들을 선정한다.<br>
마지막으로 매개변수가 없는 build 메서드를 호출해 (보통은 불변) 객체를 얻는다.<br>
